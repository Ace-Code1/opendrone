...
def main():
    ...
    longi=113.952271
    lati=22.53701
    longitude=int(math.radians(longi)*10000000)
    print(longitude)
    print(hex(longitude))   #0x12f7927   #\x30\x79\x2f\x01
    #-----------------------------------try to encode longitude
    ...
    droneid=droneid+'DroneID is crap?' #ssid
    droneid=droneid+'\xb0\x78\x5b\x00' #longitude (backwards)
    droneid=droneid+'\x29\xeb\xc2\xfe' #latitude (backwards)
    droneid=droneid+chr(250)+'\x00' #altitude
    droneid=droneid+chr(27)+'\x00' #height
    droneid=droneid+chr(216)+'\x00' #v_north
    droneid=droneid+chr(171)+'\x00' #v_east
    droneid=droneid+chr(59)+'\x00' #v_up
    droneid=droneid+chr(192)+'\x00' #pitch
    droneid=droneid+chr(244)+'\x00' #roll
    droneid=droneid+chr(64)+'\x00' #yaw
    droneid=droneid+'\x0c\x05\x3c\x00' #longitude_home (backwards)
    droneid=droneid+'\x30\x79\x2f\x01' #latitude_home (backwards)
    droneid=droneid+'\x10' #product_type
    droneid=droneid+'\x06' #uuid length
    droneid=droneid+'\x31\x39\x35\x37\x34\x31\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00' #uid
